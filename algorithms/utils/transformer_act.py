...
def discrete_autoregreesive_act(
    ...
    output_action = torch.zeros(
        (batch_size, n_agent, 1), dtype=torch.long, device=tpdv["device"]
    )
...
def discrete_parallel_act_logits(
    decoder,
    obs_rep,
    obs,
    action,
...
def continuous_autoregreesive_act(
    ...
    output_action = torch.zeros((batch_size, n_agent, action_dim)).to(**tpdv)
...
def continuous_parallel_act_logits(
    decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv
):
    shifted_action = torch.zeros((batch_size, n_agent, action_dim)).to(**tpdv)
    shifted_action[:, 1:, :] = action[:, :-1, :]
