        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs, share_obs, rewards, dones, infos, _ = self.envs.step(
                    torch.tensor(actions.transpose(1, 0, 2))
                )
                obs = _t2n(obs)
            ...
            if episode % self.log_interval == 0:
                ...
                print(
                    "\n Task {} Algo {} Exp {} Seed {} updates {}/{} episodes, total num timesteps {}/{}, FPS_tmp {}/ FPS {}, remaining_time {:.1f}h/{:.1f}h.\n".format(
                        self.all_args.task,
                        self.algorithm_name,
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            ...
            if eval_episode >= self.all_args.eval_episodes:
                ...
                eval_env_infos = {
                    "eval_average_episode_rewards": eval_episode_rewards.tolist(),
                    "eval_max_episode_rewards": [
                        torch.max(eval_episode_rewards).item()
                    ],
                }
                self.log_env(eval_env_infos, total_num_steps)
                print(
                    "eval_average_episode_rewards is {}.".format(
                        torch.mean(eval_episode_rewards).item()
                    )
                )
                break
