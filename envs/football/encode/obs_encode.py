...
class FeatureEncoder:
    def __init__(self, use_sight_range ,sight_range):
        self.active = -1
        self.player_pos_x, self.player_pos_y = 0, 0
        self.use_sight_range = use_sight_range
    ...
    def encode(self, obs , use_sight_range=None):
        use_sight_range = self.use_sight_range if use_sight_range is None else use_sight_range
        ...
        if use_sight_range and ball_distance > self.sight_range:
            ball_state = np.zeros(18)
            ball_state[16] = ball_owned
            ball_state[17] = ball_owned_by_us
        else:
        ...
        left_closest_state = left_team_state[left_closest_idx].copy()
        if use_sight_range:
            off_sight = np.where(left_team_distance.reshape(-1) > self.sight_range)
            left_team_state[off_sight] = [0, 0, 0, 0, 0, 0, -1]
        ...
        if use_sight_range:
            off_sight = np.where(right_team_distance.reshape(-1) > self.sight_range)
            right_team_state[off_sight] = [0, 0, 0, 0, 0, 0, -1]
